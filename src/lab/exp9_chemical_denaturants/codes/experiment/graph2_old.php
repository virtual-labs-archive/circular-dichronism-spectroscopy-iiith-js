...
<script class='gtm'>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-W59SWTR');</script>
 ...
 function plotStartTemp(temp){
	...
	var dataLabels = [];
	for(var i = start ; i <= end ; i+=gap){
		dataLabels.push({"label": "<span style='font-size:10px;'> "+i+" M </span>", "data": d[i]});
  	}
	$.plot($("#placeholder"), 
		 dataLabels ,
	...
	var plot =  $.plot($("#placeholder"), 
			[ {label: "MRE Vs &lambda;(in nm)<br> At Concentration: <span style='font-size:20px;'> "+temp+" </span>", data: d1} ],
 ...
 function genAnalysis(flag){
	...
	var names = ["Helix","Antiparallel","Parallel","Beta-turn","Random Coil"];
	if(flag == 'none'){
		var plot =  $.plot($("#placeholder"), 
			[ 
			 {label: "User Selected MRE values at 222 nm"+" Vs Concentration", data: d}, 
			 {label: "Original MRE values at 222 nm"+" Vs Concentration", data: d1}, 
			],
			{ grid: { hoverable: true, clickable: true }
			}
			);
		...
		var plot =  $.plot($("#placeholder"), 
			[ 
			 {label: names[flag]+" Vs Concentration", data: d}, 
			],
			{ grid: { hoverable: true, clickable: true }
			}
			);
...
.boundingBox{	
 ...
 <body>
	...
	<center>
      ...
      <div style="float:left; width:550px; margin-left:52px; font-size:16px;">
	<h2 class="boundingBox">Observations</h2>
	Secondary Structure Content of Rhodopsin (%)
	...
	<tr border="0" align="center" ><td></td><td>
	  <input type="button" value="Analysis" onclick="genAnalysis('none')" class="boundingBox">
		</td>
		<td>
	  <input type="button" value="Analysis" onclick="genAnalysis(0)" class="boundingBox">
		</td>
		<td>
	  <input type="button" value="Analysis" onclick="genAnalysis(1)" class="boundingBox">
		</td>
		<td>
	  <input type="button" value="Analysis" onclick="genAnalysis(2)" class="boundingBox">
		</td>
		<td>
	  <input type="button" value="Analysis" onclick="genAnalysis(3)" class="boundingBox">
		</td>
		<td>
	  <input type="button" value="Analysis" onclick="genAnalysis(4)" class="boundingBox">
		</td>
	</tr>
